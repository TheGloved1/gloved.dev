...
const fetchLatestReleaseDownloadLink = async () => {
  ...
  const data = (await res.json()) as GithubReleaseType;
  ...
  return (
    <div className='relative flex min-h-screen flex-col bg-[#0a0a0a] text-white selection:bg-amber-500/30'>
      ...
      <PageBack
        stayTop
        className='z-50'
        btnClassName='brutal-shadow-sm border border-amber-500/50 bg-amber-500/10 text-xs text-white hover:bg-amber-500/20'
      />

      {/* Hero */}
      <section className='relative flex flex-col items-center px-4 pb-8 pt-20 sm:pb-10 sm:pt-28 lg:pt-32'>
        ...
        <p className='font-mono-industrial mt-6 max-w-2xl text-center text-sm leading-relaxed text-white/60 sm:text-base'>
      ...
      <div className='relative px-4'>
        <Tabs value={activeTab} onValueChange={setActiveTab as (value: string) => void} className='mx-auto max-w-5xl'>
          <TabsList className='mx-auto flex w-auto justify-center gap-2 bg-transparent p-0'>
            <TabsTrigger
              value={WowAdderTab.Overview}
              className={cn(
                'font-display rounded-lg px-5 py-2.5 text-xs font-bold uppercase tracking-widest transition-all duration-150 sm:text-sm',
                activeTab === WowAdderTab.Overview ?
                  'brutal-shadow-sm border border-amber-500 bg-amber-500 text-black'
          ...
          <TabsContent value='overview' className='mt-0'>
            {/* Glow divider */}
            <div className='glow-line mx-auto mt-8 w-3/4 max-w-2xl' />
                ...
                <div className='grid gap-4 sm:grid-cols-2 lg:grid-cols-3'>
                  {features.map((feature) => (
                    ...
                    >
                      ...
                      <h3 className='font-display text-sm font-bold uppercase tracking-tight text-white'>{feature.title}</h3>
                ...
                <div className='grid gap-4 sm:grid-cols-2'>
                  {differentiators.map((item) => (
                    ...
                    >
                      ...
                      <h3 className='font-display text-sm font-bold uppercase tracking-tight text-white'>{item.title}</h3>
                      <p className='font-mono-industrial mt-1 text-xs leading-relaxed text-white/50'>{item.description}</p>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  <table className='w-full'>
                    <tbody>
                      {techStack.map((item, i) => (
                        <tr key={item.label} className={i % 2 === 0 ? 'bg-amber-500/5' : 'bg-transparent'}>
                          <td className='font-mono-industrial px-5 py-3 text-xs font-semibold uppercase tracking-wider text-amber-400'>
                            {item.label}
                          </td>
                          <td className='font-mono-industrial px-5 py-3 text-xs text-white/70'>{item.value}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
