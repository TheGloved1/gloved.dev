 ...
 */
import { deleteUserData, exportThread, sync } from '$lib/global.remote';
import { now, populateOnboardingThreads, sleep, tryCatch } from '$lib/utils';
import Dexie, { type EntityTable } from 'dexie';
import { toast } from 'svelte-sonner';
import { z } from 'zod';
import type { ChatFetchOptions, ModelID } from '$lib/ai';
...
class Database extends Dexie {
	...
	async getThreads() {
		return (
			await this.threads.where('status').notEqual('deleted').sortBy('last_message_at')
		).toReversed();
	}
	...
	async exportThread(threadId: string, userId: string) {
		...
		const { error: exportError } = await tryCatch(
			exportThread({ userId, data: { thread, messages } })
		);
		if (exportError) {
	...
	async deleteAllData(userId?: string | null) {
		...
		if (userId) {
			const { error: deleteUserDataError } = await tryCatch(deleteUserData({ userId }));
	...
	async syncDexie(userId: string) {
		...
		const threads = await this.threads.toArray();
		const messages = await this.messages.toArray();
		const { data, error: dbSyncError } = await tryCatch(
			sync({ data: { threads, messages }, userId })
		);
		if (!data || dbSyncError)
			return (console.log('[SYNC] Failed to import data'), toast.error('Failed to import data'));
		const { threads: newThreads, messages: newMessages } = data;
		if (!newThreads.length && !newMessages.length)
			return console.log('[SYNC] No data found in server');
		...
		while (newlineIndex >= 0) {
			...
			if (type === 'data-status') {
				...
				if (messageId && status) {
					await dxdb.messages.update(messageId, {
						updated_at: now(),
						status: status === 'streaming' ? 'streaming' : status === 'done' ? 'done' : undefined
					});
				}
...
export async function updateMessage(
	...
	await dxdb.messages.update(message.id, {
		content: messageContent,
		updated_at: now(),
		model: model
	});
