...
export async function exportThreadToDb(
	userId: string,
	data: { messages: Message[]; thread: Thread }
) {
...
export async function dbSync(input: { userId: string; messages: Message[]; threads: Thread[] }) {
	...
	for (const dbMessage of dbMessages) {
		...
		if (!localMessage) {
			...
			new Date(dbMessage.updated_at).getTime() > new Date(localMessage.updated_at).getTime()
		) {
	...
	for (const localMessage of localMessages) {
		...
		if (!dbMessage) {
			...
			new Date(localMessage.updated_at).getTime() > new Date(dbMessage.updated_at).getTime()
		) {
	...
	for (const dbThread of dbThreads) {
		...
		if (!localThread) {
			...
			new Date(dbThread.updated_at).getTime() > new Date(localThread.updated_at).getTime()
		) {
	...
	for (const localThread of localThreads) {
		...
		if (!dbThread) {
			...
			new Date(localThread.updated_at).getTime() > new Date(dbThread.updated_at).getTime()
		) {
 ...
 */
export async function deleteUserDataDb(userId: string) {
 ...
 */
export async function addAdminDb(email: string) {
 ...
 */
export async function removeAdminDb(email: string) {
	const admins = ((await redis.get('admins')) as string[]) || [];
	if (admins.includes(email)) {
		await redis.set(
			'admins',
 ...
 */
export async function getAdminsDb() {
 ...
 */
export async function getShortenedUrlDb(id: string) {
 ...
 */
export async function setShortenedUrlDb(userId: string, id: string, url: string) {
	const urls = ((await redis.get('shortenedUrls')) as ShortenedUrls) || {};
 ...
 */
export async function getAllUserShortenedUrlsDb(userId: string) {
 ...
 */
export async function deleteSyncDb() {
	let cursor = '0';
	do {
		const [nextCursor, keys] = await redis.scan(cursor, {
			match: 'sync*',
